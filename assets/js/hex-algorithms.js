...
function hexRing(radius) {
    var results = [];
    var H = Hex.direction(4).scale(radius);//gets new hex location in direction and scalled
    for (var side = 0; side < 6; side++) {//iterates over 6 sides
        for (var step = 0; step < radius; step++) {
            results.push(H);//adds to results array
            H = H.neighbor(side);//gets new hex location in direction
        }
    }
    return results;
}